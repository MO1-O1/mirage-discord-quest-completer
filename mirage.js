    ...
    const Logger = {
        ...
        init() {
            ...
            style.innerHTML = `
                ...
                #mirage-ui {
                    position: fixed; top: 32px; right: 20px; width: 405px; height: 560px; max-height: 85vh;
                ...
                #mirage-ui.minimized #mirage-body, #mirage-ui.minimized #mirage-logs { display: none !important; }




                #mirage-head { padding: 12px 16px; background: var(--background-mod-muted, rgba(0,0,0,0.2)); flex: 0 0 auto; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-subtle, rgba(255,255,255,0.06)); cursor: grab; }
                #mirage-head.dragging { cursor: grabbing; background: var(--control-secondary-background-default, rgba(255,255,255,0.05)); }
                #mirage-title { font-weight: 700; font-size: 15px; color: var(--text-strong, #fff); display: flex; align-items: center; gap: 8px; }
                #mirage-title svg { color: var(--text-brand, #5865f2); }
                .mirage-ver { font-size: 11px; opacity: 0.6; font-weight: 500; margin-left: 2px; }




                #mirage-controls { display: flex; gap: 8px; align-items: center; }
                .ctrl-btn { cursor: pointer; transition: 0.2s; display: flex; align-items: center; gap: 4px; }
                .ctrl-hide, .ctrl-opts, .ctrl-refresh { font-size: 11px; font-weight: 600; color: var(--text-muted, #8a8e9b); }
                .ctrl-hide:hover, .ctrl-opts:hover, .ctrl-refresh:hover { color: #fff; }
                .ctrl-refresh.spinning svg { animation: spin 0.6s linear infinite; }
                .ctrl-stop { font-size: 11px; font-weight: 700; gap: 4px; padding: 3px 8px; border-radius: 4px; background: transparent; border: 1px solid #da373c; color: #da373c; }
            ...
            this.root.innerHTML = `
                <div id="mirage-head">
                    ...
                    <div id="mirage-controls">
                        <a href="${CONFIG.GITHUB_URL}" target="_blank" rel="noopener noreferrer" class="ctrl-btn ctrl-gh" id="mirage-gh" title="GitHub Repository" style="text-decoration:none; color:#faa61a;">⭐ GitHub</a>
                ...
                <div id="mirage-body">
                    <div id="mirage-update-banner" style="display:none; padding:6px 10px; background:rgba(88,101,242,0.15); border:1px solid rgba(88,101,242,0.4); border-radius:4px; font-size:11px; margin-bottom:6px; align-items:center; justify-content:space-between;"></div>
            ...
            try {
                ...
                if (cleanLatest !== cleanCurrent) {
                    ...
                    if (banner) {
                        ...
                        const updateMsg = isAr ? `يتوفر إصدار جديد (${latestTag})` : `New update available (${latestTag})`;
                        const viewMsg = isAr ? "عرض التحديث" : "View Update";
                        banner.innerHTML = `<span>🚀 <b>${updateMsg}</b></span> <a href="${data.html_url || CONFIG.GITHUB_URL}" target="_blank" rel="noopener noreferrer" style="color:#5865f2; font-weight:700; text-decoration:underline; margin-left:6px;">${viewMsg}</a>`;
